...
(function () {
    const CurriculoModule = (function () {
        ...
        const renderCurriculo = async () => {
            ...
            if (data) {
                ...
                const highlightedTechnologies = [
                    'SQL Server', 'SQL Server Profiler', 'MongoDB', 'ElasticSearch', 'ElasticSearch', 'DTS', 'ODI', 'Oracle', "Redis", 'Migrations', 'EF', 'EF Core', 'Dapper', 'DAO', 'ADO.NET'
                ];
                ...
                container.appendChild(createSection("experience", buildExperience(data, highlightedTechnologies)));
        ...
        const buildExperience = (data, highlightedTechnologies) => {
            ...
            data.experiencias.forEach(job => {
                experienceHTML += `
                    ...
                    const highlightClass = highlightedTechnologies.includes(tech) ? 'highlight' : '';
                    return `<span class="${highlightClass}">${tech}</span>`;
                }).join('')}
                        </div>
                    </div>
                `;
